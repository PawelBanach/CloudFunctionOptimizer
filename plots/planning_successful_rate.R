...
ggplot(data=budget_03, aes(x=Deadline, y=Success.Rate, fill=Algorithm)) + geom_bar(stat="identity", position=position_dodge()) + geom_text(aes(label=Success.Rate), vjust=1.6, color="black", position = position_dodge(0.9), size=8) + ylab("Planning Success Rate (%)") + theme(plot.title = element_text(hjust = 0.5)) + theme(axis.title=element_text(size=36)) + theme(axis.text.y = element_text(size=30)) + theme (axis.text.x = element_text(size=30)) + theme(legend.text = element_text(size = 30), legend.title = element_text(size = 30))
